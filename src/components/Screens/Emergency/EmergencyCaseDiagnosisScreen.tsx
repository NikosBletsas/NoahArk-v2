...
const DIAGNOSIS_STEPS_CONFIG: DiagnosisStep[] = [
  ...
  {
    key: "historyTraumaVitalsSkin",
    label: "History/Trauma/Vitals/Skin",
    component: HistoryTraumaVitalsSkinForm,
  },
  { key: "generalSigns", label: "General Signs", component: GeneralSignsForm },
  {
    key: "surgicalNeurologicSigns",
    label: "Surgical/Neurologic Signs",
    component: SurgicalNeurologicSignsForm,
  },
  {
    key: "neurologicSigns",
    label: "Neurologic Signs",
    component: NeurologicSignsForm,
  },
  {
    key: "cardiorespPsychSigns",
    label: "Cardioresp./Psych. Signs",
    component: CardiorespPsychSignsForm,
  },
];
...
const EmergencyCaseDiagnosisScreen: React.FC = () => {
  ...
  const { submitEmergencyCase, isSubmitting } = useEmergencyCase();
  const { resetFormData } = useEmergencyCaseStore();
  ...
  useEffect(() => {
    return () => {
      // Only reset if we're navigating away without submitting
    };
  }, []);
  ...
  const handleSubmitToDashboard = () => {
    // Early submit - show confirmation modal
    setShowConfirmModal(true);
  };
  ...
  const handleCancel = () => {
    resetFormData(); // Clear form data when canceling
  ...
  const inactiveItemHoverBgClass =
    isMidnightTheme || currentThemeKey === "black"
      ? "hover:bg-white/15"
      : "hover:bg-black/10";
  ...
  const sidebarContent = (isMobile?: boolean) => (
    <>
      <div
        ...
        } mb-3 sm:mb-4 md:mb-5 lg:mb-6 border-b ${
          isMidnightTheme || currentThemeKey === "black"
            ? "border-slate-600"
            : "border-white/20"
      ...
      >
        <div className="flex items-center space-x-2 md:space-x-3">
          <div>
            <h1
              className={`text-sm sm:text-base md:text-lg lg:text-xl font-semibold ${theme.textOnAccent}`}
            >
              {isMobile ? "Diagnosis Steps" : "NOAH Diagnosis"}
            </h1>
          </div>
        </div>
        {isMobile && (
  ...
  return (
    <div className={`min-h-screen flex bg-gradient-to-br ${theme.background}`}>
      ...
      <main className="flex-1 md:ml-72 lg:ml-80 flex flex-col overflow-hidden">
        <div
          className={`md:hidden sticky top-0 ${
            theme.card
          } backdrop-blur-lg p-2.5 sm:p-3 flex items-center justify-between border-b ${
            isMidnightTheme || currentThemeKey === "black"
              ? "border-gray-700/60"
              : "border-white/20"
          } z-10`}
        >
          <button
            ...
            className={`p-1.5 sm:p-2 rounded-md hover:bg-black/10 ${
              isMidnightTheme || currentThemeKey === "black"
                ? "text-white"
                : "text-slate-700"
            }`}
          ...
          <h2
            className={`text-sm sm:text-base font-semibold ${theme.textPrimary} truncate mx-2`}
          >
          ...
          >
            <div
              className={`w-4 h-4 sm:w-5 sm:h-5 bg-gradient-to-r ${theme.primary} rounded-sm sm:rounded-md`}
            ></div>
          </button>
        </div>
        ...
        <div className="p-3 sm:p-4 md:p-6 lg:p-8 flex-grow overflow-y-auto">
            ...
            <div className="flex-1 max-w-2xl mr-6">
              ...
              >
                <div
                  ...
                  style={{
                    width: `${
                      ((currentStepIndex + 1) / DIAGNOSIS_STEPS_CONFIG.length) *
                      100
                    }%`,
                  }}
                >
          ...
          <div
            className={`${theme.card} backdrop-blur-lg rounded-xl shadow-2xl border border-white/20 flex flex-col`}
          >
            <div
              className={`bg-gradient-to-r ${theme.accent} ${theme.textOnAccent} p-2.5 sm:p-3 md:p-3.5 lg:p-4 rounded-t-lg sm:rounded-t-xl flex items-center justify-between`}
            >
        ...
        <div
          className={`bg-gradient-to-r ${
            theme.accent
          } p-3 sm:p-4 md:p-5 lg:p-6 sticky bottom-0 z-10 border-t ${
            isMidnightTheme || currentThemeKey === "black"
              ? "border-gray-600/50"
              : "border-white/20"
          } backdrop-blur-lg`}
          ...
          <div className="max-w-4xl mx-auto flex justify-end items-center space-x-2 sm:space-x-3 md:space-x-4">
            ...
            {!isLastStep && (
              <button
                ...
                className={`group relative px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2.5 lg:px-6 lg:py-3 rounded-md sm:rounded-lg font-medium bg-gradient-to-r ${theme.primary} ${theme.textOnAccent} transition-all duration-300 text-xs sm:text-sm md:text-base lg:text-lg overflow-hidden hover:scale-105 hover:shadow-lg hover:shadow-blue-500/10 active:scale-95 transform border border-white/20 opacity-90 hover:opacity-100 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100`}
            ...
            <button
              onClick={handleNext}
              disabled={isSubmitting}
              className={`group relative px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2.5 lg:px-6 lg:py-3 rounded-md sm:rounded-lg font-medium bg-gradient-to-r ${theme.primary} ${theme.textOnAccent} transition-all duration-300 text-xs sm:text-sm md:text-base lg:text-lg overflow-hidden hover:scale-105 hover:shadow-lg hover:shadow-blue-500/15 active:scale-95 transform shadow-md disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100`}
            >
              <span className="relative z-10 flex items-center space-x-2">
                <span>
                  {isSubmitting
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to submit this emergency case?"
