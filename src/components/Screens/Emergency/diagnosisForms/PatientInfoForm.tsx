  ...
  useEffect(() => {
    if (selectedPatient) {
      const newValues = {
        ...
        'patient-sex': selectedPatient.gender?.toLowerCase() === 'male' ? 'male' : 
                      selectedPatient.gender?.toLowerCase() === 'female' ? 'female' : '',
      ...
      updateFormData({
        ...
        gender: selectedPatient.gender?.toLowerCase() === 'male' ? 'male' : 
               selectedPatient.gender?.toLowerCase() === 'female' ? 'female' : '',
      });
    ...
    <div className="space-y-4 sm:space-y-6 md:space-y-7">
      {selectedPatient && (
        <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
          <span className="text-green-700 text-sm font-medium">
            ✓ Patient loaded: {selectedPatient.name} {selectedPatient.surname}
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 sm:gap-x-6 md:gap-x-7 gap-y-3 sm:gap-y-4 md:gap-y-5">
        ...
        <LabelledInput 
          ...
          onChange={(e) => handleInputChange('patient-father-name', e.target.value)}
        />
        <LabelledInput 
          label="Surname" 
          id="patient-surname" 
      ...
      <div className="mt-3 sm:mt-4 md:mt-5">
        <LabelledTextarea 
          ...
          onChange={(e) => handleInputChange('patient-other-identifier', e.target.value)}
        />
      </div>
    </div>
  );
