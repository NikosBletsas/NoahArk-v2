...
The `LoginScreen` component provides the user interface for authentication. It includes a form for entering a username and password, as well as options for offline login and accessing connectivity diagnostics.

### Structure

- **`IconButton`**: A reusable button component with an icon and a label.
- **Login Form**: A form with fields for username and password.
- **Password Visibility Toggle**: An icon button that allows the user to show or hide the password.
- **Login Buttons**:
  - **Login**: Submits the form to log in with the provided credentials.
  - **Login Offline**: Allows the user to log in without an internet connection.
- **Connectivity Buttons**:
  - **Connectivity**: Opens the `ConnectionStatusScreen` modal.
  - **Diagnostics**: Opens the `ConnectivityTestScreen` modal.
- **Logos**: Displays the NOAH and TMA logos.

### Functionality

- **State Management**:
  - Uses `useState` to manage the username, password, and password visibility.
  - Uses `useUIStore` to control the visibility of the connectivity modals.
- **Authentication**:
  - Uses the `useLogin` hook to handle both online and offline login.
  - The `handleLogin` function calls `login` with the username and password.
  - The `handleLoginOffline` function calls `loginOffline`.
...
- **Loading State**: Disables the form and shows a loading indicator while the login is in progress.
